...
const props = defineProps<{
  originalViewBox?: IRectangle | undefined
}>()
...
const cursorInfo = computed(() => {
  if (!['scale', 'rotate'].includes(canvasStore.editTransformType.value)) return
  return {
...
const showAxis = computed(() => settings.showAxis)
const scale = canvas.scale
const viewSize = canvas.viewSize
const viewBox = canvas.viewBox
const dragRectangle = canvas.draggedRectangle
const canvasMode = canvasStore.canvasMode
const popupMenuItems = computed(() => popupMenuList.list.value)
const wheel = canvas.wheel
const changeMode = handleChangeMode
const commandExamList = canvasStore.commandExamList
</script>
